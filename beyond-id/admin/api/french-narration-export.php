...
$in=json_decode((string)file_get_contents('php://input'),true);$text=trim((string)($in['text']??''));$locale=(string)($in['locale']??'en-US');$date=(string)($in['publish_date']??date('Y-m-d'));$library='beyond-french';
